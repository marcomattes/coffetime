...
(() => {
  ...
  interface AppState {
    ...
    /* Last invite code the server confirmed, used to build the registration
       link for an NFC tag. Empty until /api/admin/settings has answered --
       the field above it may hold an edit that was never saved. */
    invite: string;
    /* Server-declared minimum for the admin password, so the client-side
       check cannot drift away from Passwords::MIN_LENGTH. The literal is only
       the value used before /api/admin/settings has answered. */
    passwordMinLength: number;
  }
  ...
  /* Normalizes one parsed JSON value into a QueueEntry, or null when it is
     ...
     or plain corruption. */
  function toQueueEntry(item: unknown): QueueEntry | null {
  ...
  function rememberInstallDismissed(dismissed: boolean): void {
    try {
      if (dismissed) {
  ...
  function updateInstallUi(): void {
    ...
    const button = el<HTMLButtonElement>('btn-install');
    ...
    if (isIos()) {
      ...
      text(
        el('install-text'),
        'Reminders and the full-screen app need Coffee Time on your home screen. On iPhone and iPad that takes three taps:'
      );
      return;
    }
    hide();
  ...
  function updateReminderUi(backgroundChecks?: boolean): void {
    ...
    const help = el<HTMLButtonElement>('btn-notify-install');
    if (!status || !button) {
      return;
    }
    if (help) {
  ...
  const PAGES = ['coffee', 'users', 'settings'] as const;
   ...
   */
  let requestedPage: Page = hashPage();

  function syncPage(): void {
    const admin = state.me !== null && state.me.admin === true;
    const active: Page = admin ? requestedPage : 'coffee';
  ...
  function initAdminNav(): void {
    const nav = el('admin-nav');
    if (nav) {
      nav.addEventListener('click', (event) => {
        const origin = event.target as HTMLElement | null;
        const item = origin?.closest?.<HTMLElement>('[data-page]') ?? null;
  ...
  function renderAdminPasswordState(isSet: boolean, setAt?: number): void {
    const stateNode = el('admin-password-state');
    const removeButton = el<HTMLButtonElement>('btn-admin-password-remove');
  ...
  async function saveAdminSettings(): Promise<void> {
    ...
    const priceValue = Number.parseFloat(el<HTMLInputElement>('admin-price-input')!.value);
  ...
  async function decryptAdminName(user: AdminUser): Promise<void> {
    ...
    if (!state.adminKey || !user.nameEncrypted?.startsWith(prefix)) return;
  ...
  async function loginWithPassword(): Promise<void> {
    const button = el<HTMLButtonElement>('btn-login-password');
    const errorNode = el('auth-error');
    text(errorNode, '');

    const firstName = el<HTMLInputElement>('pw-firstname-input')!.value.trim();
    const lastName = el<HTMLInputElement>('pw-lastname-input')!.value.trim();
    const passwordInput = el<HTMLInputElement>('pw-password-input')!;
  ...
  async function initSetup(): Promise<void> {
    ...
    const priceValue = Number.parseFloat(el<HTMLInputElement>('setup-price')!.value);
    if (!Number.isFinite(priceValue) || priceValue <= 0 || priceValue > 1000) {
  ...
  async function saveAdminPassword(): Promise<void> {
    const button = el<HTMLButtonElement>('btn-admin-password');
    const statusNode = el('admin-password-status');
    const passwordInput = el<HTMLInputElement>('admin-password-input')!;
    const repeatInput = el<HTMLInputElement>('admin-password-repeat')!;
    text(statusNode, '');
    ...
    busy(button, true);
    try {
      const data = await api<AdminPasswordResponse>('/api/admin/password', { password: password });
      passwordInput.value = '';
      repeatInput.value = '';
  ...
  async function removeAdminPassword(): Promise<void> {
    const button = el<HTMLButtonElement>('btn-admin-password-remove');
    const statusNode = el('admin-password-status');
    text(statusNode, '');
    ...
    busy(button, true);
    try {
      const data = await api<AdminPasswordResponse>('/api/admin/password', { remove: true });
      el<HTMLInputElement>('admin-password-input')!.value = '';
      el<HTMLInputElement>('admin-password-repeat')!.value = '';
      renderAdminPasswordState(data.passwordSet === true, data.passwordSetAt);
  ...
  function checkInviteLink(): void {
    ...
    const input = el<HTMLInputElement>('invite-input');
    if (input) {
      input.value = code;
    }
  ...
  /*
   * The two links above are what a sticker on the machine should contain, so
   * the admin should not have to type them into a separate tag-writing app.
   * Web NFC (Chrome on Android, secure context only) can write them directly;
   * everywhere else the card still shows the URLs so they can be copied into
   * whatever tool is at hand.
   *
   * A tag is only a link. The booking tag is not a credential: it opens the
   * app and books for whoever is signed in on that phone, and only because a
   * tag opens the app without a referrer (see checkPendingBook). The
   * registration tag carries the invite code and is therefore exactly as
   * secret as the code itself.
   */
  ...
  function ndefWriterCtor(): NdefWriterCtor | null {
  ...
  function registrationLink(invite: string): string {
    return window.location.origin + '/?invite=' + encodeURIComponent(invite);
  }

  function renderNfcUi(): void {
    const bookUrl = el('nfc-book-url');
    const inviteUrl = el('nfc-invite-url');
    const bookButton = el<HTMLButtonElement>('btn-nfc-book');
    const inviteButton = el<HTMLButtonElement>('btn-nfc-invite');
  ...
  async function writeNfcTag(url: string, label: string): Promise<void> {
  ...
  function nfcErrorText(error: unknown): string {
