...
- Each booking stores the price at booking time, so later price changes never reprice coffees already booked.
- Personal 28-day history with a today count and a chart, alongside the anonymous ranking.
- An administrator can book a payment against any account to settle a tab.
- A first-run setup wizard: the admin keypair is generated in the browser, the private key is only offered for download, and the public key, price, and invite code are stored without touching `config.php`.
...
- SQLite by default; optional MySQL/MariaDB.
- TypeScript frontend, compiled to plain JavaScript the server ships (no Node runtime in production).
...
Set `rpId` to the host without a port and `origin` to the complete origin once you deploy beyond localhost. Production WebAuthn deployments require HTTPS.
...
If `config.php` already sets `adminPublicKey`, the setup wizard is skipped. Once any of `priceCents`, `invite`, `adminPublicKey`, or `namePepper` has been set through the app (wizard or settings screen), the stored value takes precedence over `config.php` on every later request — see Settings precedence below.

## Configuration

`config.php` returns an array. Important settings are `priceCents`, `invite`, `admins` (user IDs as strings), `rpId`, `origin`, `dbPath` (or `db`, see Database), `adminPublicKey`, and the secret `namePepper`. Keep the SQLite database and all private keys outside `public/`.
...
Add the account ID to `admins` (or rely on the first-registered-user rule), sign in, and select `admin-private.pem` in the Administration section. JavaScript imports it as a non-extractable Web Crypto key and decrypts API ciphertexts in memory. No request containing the key or plaintext names is made. From the same screen an administrator can book a payment against a user's tab and export the decrypted roster as CSV, both without the key ever leaving the browser.
