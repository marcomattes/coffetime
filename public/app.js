...
(() => {
    ...
    function clearReminderBadge() {
        ...
        navigator.serviceWorker.ready
            .then((registration) => typeof registration.getNotifications === 'function'
     ...
     */
    function renderPaypal(handle, balanceCents) {
        const card = el('paypal-card');
        const link = el('paypal-link');
        if (!card || !link) {
            return;
        }
    ...
    function renderAdminSettings(settings) {
        ...
        const paypalInput = el('admin-paypal-input');
     ...
     */
    async function deleteUser(user, button, node) {
    ...
    async function saveAdminSettings() {
        ...
        const paypalRaw = el('admin-paypal-input').value;
    ...
    async function initSetup() {
        ...
        const status = el('setup-status');
        const setupToken = el('setup-token').value.trim();
        if (!setupToken) {
    ...
    function consumePendingBook() {
        ...
        const confirm = el('btn-book-confirm');
        if (confirm) {
            confirm.focus();
        }
    }
    function dismissBookConfirm() {
        const card = el('book-confirm');
        if (card) {
            card.hidden = true;
        }
